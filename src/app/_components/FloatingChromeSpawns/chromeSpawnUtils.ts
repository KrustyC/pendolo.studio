...
export function blocksChromeSpawn(e: PointerEvent): boolean {
  ...
  const touchesInteractive = (el: Element | null) =>
    Boolean(el?.closest(INTERACTIVE_ROOT_SELECTOR));
  ...
  if (
    ...
    (touchesInteractive(e.target) || inFixedChrome(e.target))
  ) {
    return true;
  }
  ...
  if (
    ...
    (touchesInteractive(top) || inFixedChrome(top))
  ) {
    return true;
  }
...
export function isPointerInHeroSection(
  clientX: number,
  clientY: number
): boolean {
