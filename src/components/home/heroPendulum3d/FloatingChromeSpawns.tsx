...
import dSvg from "../../../../d.svg?raw";
import eSvg from "../../../../e.svg?raw";
import lSvg from "../../../../l.svg?raw";
import nSvg from "../../../../n.svg?raw";
import oSvg from "../../../../o.svg?raw";
import oAltSvg from "../../../../o_1.svg?raw";
import pSvg from "../../../../p.svg?raw";
import { blocksChromeSpawn, isPointerInHeroSection } from "./chromeSpawnUtils";

/**
 * Click-spawned helium bubble letters:
 * - Each click creates the next p/e/n/d/o/l/o letter in the hero.
 * - SVG silhouettes become bevelled foil geometry, then float upward with helium-like drift.
 */
...
function createBalloonLetterGeometry(svgMarkup: string): LetterGeometry {
  ...
  const geometry = mergeGeometries(geometries, false) ?? geometries[0] ?? new THREE.BoxGeometry(1, 1, 0.12);
  ...
  // SVG coordinates are y-down; flip into Three's y-up space after centering.
...
function BubbleSvgLetter({
  ...
  useFrame((_, dtRaw) => {
    ...
    if (simEnabled) {
      ...
      if (!expiredRef.current && (item.basePos.y > CULL_Y_TOP || item.basePos.lengthSq() > CULL_RADIUS_SQ)) {
  ...
  return (
    <group ref={groupRef} renderOrder={18}>
      <mesh geometry={letterGeometry.geometry} material={foilMaterial} frustumCulled={false} renderOrder={18} />
    </group>
  );
}
...
export function FloatingChromeSpawns() {
  ...
  useEffect(() => {
    const projectToWorld = (clientX: number, clientY: number): THREE.Vector3 | null => {
      ...
      // Camera-facing plane through the world origin keeps clicks mapped to what feels like the screen.
